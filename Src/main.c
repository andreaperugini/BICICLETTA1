...
float32_t H_data[measure_dim*state_dim] = {1,0,
											0,1};

float32_t Q_data[state_dim*state_dim]= {0.00005/2.5,0,
		0,0.00004};

float32_t R_data[measure_dim*measure_dim] = {0.0220,0,
											0,0.4655};
float32_t P_data[state_dim*state_dim]={100, 0,
										0,100};
...
int main(void) {
	...
	tune_PID(&pid_steering_torque, 20, 22, 0);
	//*************************
	...
	kalman_filter_init(&kf, &A_data, &B_data, &H_data, &Q_data, &R_data, &P_data, &K_data, &x_data);
